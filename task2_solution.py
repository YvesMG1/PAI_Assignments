...
class SWAGInference(object):
    ...
    def __init__(
        ...
        pred_prob_all: torch.Tensor = np.zeros(140),
        pred_prob_max: torch.Tensor = np.zeros(140), 
        pred_ys_argmax: torch.Tensor = np.zeros(140),
        pred_ys: torch.Tensor = np.zeros(140),
        ...
        swag_learning_rate: float = 0.045, # if constant learning rate
        swag_update_freq: int = 1,
        deviation_matrix_max_rank: int = 15, #20 from the SWAG paper, 15 in their template
        ...
        self.parameters = self._create_weight_copy()  # cnn weights
        ...
        # deque: object like a Python list. Designed for memory-efficient appends
        # I think we use this to keep track of the columns in the deviation matrix
        self.deviation_deque = collections.deque(maxlen=deviation_matrix_max_rank)
    ...
    def update_swag(self) -> None:
        ...
        self.swag_count += 1 #update the number of times swag has been updated

        # I think we have to update the swag_count before we run the moment updates
        # To be faithful to the original algorithm

        # SWAG-diagonal
        # TODO(1): update SWAG-diagonal attributes for weight `name` using `current_params` and `param
        for name, param in current_params.items():
            self.mean_params[name] = (self.mean_params[name] * self.swag_count + param) / (self.swag_count + 1) 
            self.sq_params[name] = (self.sq_params[name] * self.swag_count + param.pow(2)) / (self.swag_count + 1) 
        ...
        if self.inference_mode == InferenceMode.SWAG_FULL:
            ...
            # Subtract running mean at time i from network parameters at time i
            deviation_dict = {name:current_params[name] - self.mean_params[name] for name in current_params.keys()}

            # Add the dictionary of differences to the deviation 'matrix'
            # One dictionary per i
            # automatically bounded to be of length maximum K by the datastructure of a deque
            self.deviation_deque.append(deviation_dict)
    ...
    def fit_swag(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        lr_scheduler = SWAGScheduler(
            ...
            lr_min = 0.01,
            lr_max = 0.05, # Empirically speaking, 0.1 is too high 
            cycle_length = 5
        )

        # TODO(1): Perform initialization for SWAG fitting

        self.mean_params = self._create_weight_copy() #Yves' adaptation
        self.sq_params = self._create_weight_copy()

        #self.mean_params = {name: param.detach().clone() for name, param in self.network.named_parameters()}
        #self.sq_params = {name: param.detach().clone().pow(2) for name, param in self.network.named_parameters()}

        self.swag_count = 0  # Keep track of how many times SWAG has been updated

    
        self.network.train()

        with tqdm.trange(self.swag_epochs, desc="Running gradient descent for SWA") as pbar:  # this is just code to implement displaying a progress bar
            # pbar refers to the progress bar. In this case, it is self.swag_epochs that form the progress bar
            pbar_dict = {}
            for epoch in pbar: #ie, epoch in self.swag_epochs
                average_loss = 0.0
                ...
                # implement swag update at the rate of swag_update_freq
    ...
    def predict_probabilities_swag(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        """
        # setting to evaluate the cnn, rather than train
        self.network.eval() 
        ...
        for _ in tqdm.trange(self.bma_samples, desc="Performing Bayesian model averaging"):
            ...
            # sample parameters method doesn't return anything - rather, updates the self.network automatically to be the posterior
         ...
         #   raise NotImplementedError("Sample network parameters")
            ...
            model_sample_predictions = []  #list of tensors
            with torch.no_grad():
                for (batch_xs,) in loader: #for each batch in the loader
                    #for each sample, we get one prediction
                    outputs = self.network(batch_xs) # evaluating network on the data sample
                    probabilities = torch.nn.functional.softmax(outputs, dim=-1) # softmax to convert to probabilities
                    # each tensor in model_sample_predictions contains probabilities for one batch of data from the loader
                    model_sample_predictions.append(probabilities) 

            # After processing all batches, concatenate all batch predictions from that model into a single tensor
            all_samples_predictions = torch.cat(model_sample_predictions, dim=0)
            # add the concatenated predictions from the model onto per_model_sample_predictions
            per_model_sample_predictions.append(all_samples_predictions)
      ...
      #  raise NotImplementedError("Aggregate predictions from model samples")
        stacked_predictions = torch.stack(per_model_sample_predictions, dim=0)
        # average probability predictions across the models
    ...
    def sample_parameters(self) -> None:
        ...
        # does this for each paramater. So, we sample each parameter from a gaussian
        for name, param in self.network.named_parameters():
            # SWAG-diagonal part
            z_1 = torch.randn(param.size())  # randomness
            # TODO(1): Sample parameter values for SWAG-diagonal

            current_mean = self.mean_params[name] 
            variance = self.sq_params[name] - current_mean.pow(2) 
            current_std = torch.sqrt(variance.clamp(min=1e-10))
            
            assert current_mean.size() == param.size() and current_std.size() == param.size() 
            
            # Diagonal part
            if self.inference_mode == InferenceMode.SWAG_DIAGONAL:
                sampled_param = current_mean + current_std * z_1 # sampled parameter 

            # Full SWAG part
            elif self.inference_mode == InferenceMode.SWAG_FULL:
                # TODO(2): Sample parameter values for full SWAG
                # First part
                sampled_param = current_mean + (1/sqrt(2)) * current_std * z_1 
                
                # Sample K values from N(0,1)
                z_2 = torch.randn(self.deviation_matrix_max_rank)

                # Replicate the matrix-vector multiplication with arbitrary tensor shapes
                for i,d in enumerate(self.deviation_deque):
                    self.deviation_deque[i][name] = d[name] * z_2[i]
                
                new_term = torch.stack([d[name] for d in self.deviation_deque], dim=0).sum(dim=0)
                sampled_param += (1/(sqrt(2 * (self.deviation_matrix_max_rank - 1)))) * new_term 
...
class SWAGScheduler(torch.optim.lr_scheduler.LRScheduler):
    ...
    def calculate_lr(self, current_epoch: float, old_lr: float) -> float:
        ...
        # Option (a): constant learning rate
        return old_lr

        # Option (b): cyclical learning rate
        # Learning rate varies linearly between the lr_min and lr_max, changing at each epoch
        # i =  (math.floor(current_epoch) % self.cycle_length) / self.cycle_length
        # new_lr = i * self.lr_min + (1-i) * self.lr_max 
        # return new_lr
...
def evaluate(
    ...
    swag.pred_prob_all = pred_prob_all
    swag.pred_prob_max = pred_prob_max
    swag.pred_ys_argmax = pred_ys_argmax
    swag.pred_ys = pred_ys

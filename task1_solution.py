import os
import typing
from sklearn.gaussian_process.kernels import RBF, WhiteKernel, Matern, ExpSineSquared, RationalQuadratic, DotProduct
import numpy as np
...
class Model(object):
    ...
    def __init__(self, alpha = 0, area_penalty = 0.25, uncertainty_penalty = 2):
        """
        Initialize your model here.
        We already provide a random number generator for reproducibility.
        """
        self.rng = np.random.default_rng(seed=0)
        self.kernel = WhiteKernel() + Matern()
        self.model = None
    ...
    def make_predictions(self, test_x_2D: np.ndarray, test_x_AREA: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        test_x_AREA = test_x_AREA.astype(bool)
        ...
        # Add offset for city areas
        predictions[test_x_AREA] += self.area_penalty

        # Add higher offset for city areas with high uncertainty
        predictions[test_x_AREA & (gp_std > 0.5)] += self.uncertainty_penalty
    ...
    def fitting_model(self, train_y: np.ndarray,train_x_2D: np.ndarray):
        ...
        self.model = GaussianProcessRegressor(kernel=self.kernel, alpha=self.alpha, n_restarts_optimizer=5, normalize_y=True, random_state=42b)
        
        # Reduce the training set
        train_x_2D, train_y = reduce_train_set(train_x_2D, train_y, k = 5000)
...
def reduce_train_set(train_x_2D: np.ndarray, train_y: np.ndarray, k = 3000) -> typing.Tuple[np.ndarray, np.ndarray]:

    # Fit a KMeans model to the training set
    km = KMeans(n_clusters=k, random_state=42)
    km.fit(train_x_2D)

    # Reduce the training set to the cluster centers
    train_x_2D = km.cluster_centers_

    # Determine the mean pollution level for each cluster
    train_y = np.array([train_y[km.labels_ == i].mean() for i in range(k)])
